...
def favouriteDuo(a1,a2)->dict:
    conn = db.connect()
    query_results = conn.execute("SELECT s.name, a.name FROM Songs s JOIN Artists a ON s.ArtistId =a.ArtistId WHERE a.name = '{}' UNION SELECT s2.name, a2.name FROM Songs s2 JOIN Artists a2 ON s2.ArtistId = a2.ArtistId WHERE a2.name = '{}'".format(a1,a2)).fetchall()
    conn.close()
    song_list = []
    for result in query_results:
